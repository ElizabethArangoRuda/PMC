#' Peat Spread Index (PSI)
#'
#' The Peat Spread Rate Index (PSI) integrates the Peat Moisture Code (PMC) and the Initial Spread Index (ISI)
...
#' @param year_to_plot Optional. Specify a year (as a string or numeric) to generate a plot of PSI values for that year.
#'
#' @return A dataframe containing all original columns along with two new columns \code{PSI} and \code{PSI_scaled}, representing the calculated peat spread index.
...
#'   ISI = c(0.2, 0.1, 0.1)
#' )
#' PSI(input_data, year_to_plot = "2014")
PSI <- function(input_data, year_to_plot=NULL){
  ...
  plot <-
    ggplot2::ggplot(input_data, ggplot2::aes(x=Date, y=PSI_scaled)) +
    ggplot2::geom_line()+
    ggplot2::geom_point()+
    ...
    ggplot2::labs(x = "Date", y = expression(PSI_scaled))+
    ggplot2::theme(
